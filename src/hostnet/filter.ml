...
module Make(Input: Sig.VMNET) = struct
  ...
  let filter valid_subnets valid_sources next buf =
    ...
    | Ok ( {Ethif_packet.ethertype = IPv4}, payload) ->
      ...
      then next buf
        ...
        let dst = Ipaddr.V4.to_string @@ Ipaddr.V4.of_int32 @@ Ipv4_wire.get_ipv4_dst payload in
        let body = Cstruct.shift payload Ipv4_wire.sizeof_ipv4 in
        begin match Ipv4_packet.Unmarshal.int_to_protocol Ipv4_wire.(get_ipv4_proto payload) with
          | Some `UDP ->
